...
- 🚧 **Сущности/AI**: SoA ECS-хранилище реализовано (`aether-entity`: generational `EntityId`, параллельные колонки компонентов, пакетная интеграция). Flow-Field навигация, кэшируемый A\* и пакетный спавнер ещё 📋. Для превью-сервера также есть одна сущность `Player`.
