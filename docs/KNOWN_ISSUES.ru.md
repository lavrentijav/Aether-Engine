...
>
> Последняя ревизия: 2026-07-28

---

### A. Текущие известные проблемы (ранняя альфа)
1. **Игровые подсистемы не построены.** Хранилище, базовая физика, генерация мира и API ядра есть, но редстоун, асинхронное освещение, ECS-сущности/AI и полный поэтапный конвейер физики ещё не реализованы. Освещение пока использует фолбэк `FullBright` (всегда макс. свет).
...
| **Освещение** | Фолбэк `FullBright` — каждый блок освещён по максимуму | Асинхронный flood-fill по ячейкам со слиянием в safe-points |
